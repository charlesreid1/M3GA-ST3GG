...
- **"The rendered post"** (Slack, Discord, iMessage bodies, most chat UIs) — canonical form is what the user's eyes see. Emoji get colon-canonicalized on the wire (Slack stores raw codepoints inside `blocks` but exposes `:red_circle:` in the flat `.text` field; other clients drop the raw form entirely), attached image files get re-served without EXIF/XMP/IPTC and without named PNG text chunks (tEXt/iTXt/zTXt), and the message body itself gets normalized — Slack in particular strips whitespace-steg (trailing/repeated spaces) and Unicode-tag-block invisible-ink. Kills: emoji tag sequences on paste, emoji-attached data if the consumer reads the flat text field, image metadata chunks, EXIF/XMP, trailing-whitespace SNOW on paste (survives as a snippet), invisible-ink on paste (survives as a snippet).
...
- If the user says "over Slack", **ask which of the three Slack transports** — they behave very differently:
  - **`slack_upload`** (file attached to a channel): PNG LSB is byte-identical (all 6 channel/bit/strategy variants tested SURVIVE). PNG private/unknown-type chunks pass through. Named ancillary text chunks (tEXt/iTXt/zTXt) are STRIPPED, as is EXIF/XMP/IPTC. JPEG (baseline and progressive) gets re-encoded — F5/JSteg dies, generic DCT sometimes survives if tuned to match Slack's re-encoder but treat that as luck. WebP and TIFF also re-encoded. BMP/GIF-static/SVG/plain-text upload byte-identical. Trailing bytes past IEND stripped; past EOI effectively destroyed by the JPEG re-encode.
  - **`slack_paste`** (text in the message body via `chat.postMessage`): 13 of 15 text techniques SURVIVE — zero-width, cyrillic/CJK homoglyph, variation selectors, combining CGJ, directional overrides, Hangul filler, mathbold, braille, capitalization, confusable, emoji substitution (🔴/🔵), and even skintone modifier bits when the consumer reads them right. **RECODED on paste:** whitespace steg (Slack normalizes trailing/repeated spaces) and invisible-ink Unicode tag block (Slack drops most tag chars from the message body). **Emoji-attached data (VS-16, skintone bits, tag sequences)** — the raw codepoints are preserved inside `blocks`, but if the receiver reads the rendered `.text` field they get `:colon_form:` (`:+1::skin-tone-3:`, `:red_circle:`) with the modifiers/tag chars compressed away. So paste-side emoji steg only works when both sides agree to walk `blocks`.
  - **`slack_snippet`** (text uploaded as a .txt/code snippet, `files.upload snippet_type=text`): raw file bytes end-to-end, no rendering pipeline. **Every text technique tested SURVIVES**, including the two that RECODE on paste (whitespace, invisible-ink). Strictly stronger channel for text stego whenever "attach as a snippet" is acceptable UX.
  - Practical picks: PNG LSB for image hides, `slack_snippet` for any text hide that has to survive intact, `slack_paste` only when the technique is on the confirmed-survive list AND the receiver won't be relying on `.text` for emoji-carried data. See `TRANSPORT_MATRIX.md` for the cell-level scoreboard.
  - **Retrieval gotcha for anyone building a `slack_paste` consumer:** `conversations.history` returns a `.text` field that is (a) rendered in `:colon_form:` and (b) capped at 4000 chars. Do not compare against `.text` — walk `blocks[].rich_text_section.elements[]` and use the `unicode` field on emoji nodes to reproduce the on-the-wire content.
...
- **Over Slack** → three sub-transports, ask which one. If you can't ask, default to snippet for text and file upload for images.
  - **`slack_upload`** (image or arbitrary file attached to a channel). **Kills:** named PNG text chunks (tEXt/iTXt/zTXt), EXIF/XMP/IPTC on JPEGs, JPEG F5/JSteg (Slack re-encodes JPEGs), trailing bytes past IEND (JPEG EOI trailer is destroyed by the JPEG re-encode too), WebP and TIFF (re-encoded). **Survives:** PNG LSB (IDAT byte-identical, all 6 channel/bit/strategy variants tested), PNG private/unknown-type chunks (Slack strips only the named text chunks), BMP / GIF-static / SVG / plain-text uploads (byte-identical), PNG matryoshka and SPECTER (inherit LSB byte-identity). Practical picks: PNG LSB for image hides; private PNG chunk if metadata is the goal.
  - **`slack_paste`** (text in the message body via `chat.postMessage`). **Kills:** whitespace-steg (Slack normalizes trailing/repeated spaces), invisible-ink Unicode tag block (Slack drops tag chars from the body), emoji tag sequences. **Ambiguous:** emoji VS-16 and skintone modifier bits — Slack stores the raw codepoints inside `blocks` but any consumer that reads the flat `.text` field gets `:colon_form:` with the modifiers gone; only reliable if both sides walk `blocks`. **Survives:** zero-width, cyrillic and CJK homoglyphs, variation selectors, combining CGJ, directional overrides, Hangul filler, mathbold, braille, capitalization, confusable, emoji substitution (🔴/🔵 block), zalgo, leet.
  - **`slack_snippet`** (text uploaded as `files.upload snippet_type=text`). **Survives everything text-family tested** — including the two paste-killers (whitespace, invisible-ink). Raw bytes end-to-end, no rendering pipeline, no length cap. Strictly stronger than paste for text stego; recommend it whenever "attach as a snippet" is acceptable UX.
