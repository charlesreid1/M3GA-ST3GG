...
def _resolve_knowledge_root() -> Path:
    """Locate the knowledge corpus in both dev and installed layouts.

    - Installed wheel: `pyproject.toml` force-includes `knowledge/` as
      `m3gast3gg/_knowledge/`, right next to this file's package dir.
    - Dev tree: the source `knowledge/` still lives at the repo root, four
      levels above `src/m3gast3gg/mcp/knowledge.py`.

    Fall back to the dev path even when neither exists so error messages
    stay readable.
    """
    package_local = Path(__file__).resolve().parent.parent / "_knowledge"
    if package_local.exists():
        return package_local
    repo_root = Path(__file__).resolve().parents[3] / "knowledge"
    return repo_root
