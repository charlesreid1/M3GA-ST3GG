...
**Text transforms (`stegg_list_transforms` / `stegg_inspect_transform` / `stegg_encode_transform` / `stegg_decode_transform` / `stegg_auto_decode`)** — reversible reshaping of a *bare* string, distinct from text steg (no cover, no secret). 20 transforms across 6 categories: ciphers (caesar, rot13, atbash, vigenere — no encryption; CTF-grade), encodings (base64, base32, hex, binary, ternary, ascii85, morse, url, quoted-printable), unicode (fullwidth, zalgo), concealment bridges (homoglyph, invisible-text, zero-width — wrap the steg primitives into a single-arg transform), format (reverse), visual (leetspeak). Reach for `stegg_auto_decode` when handed a mystery string and told "figure out what encoding this is" — the universal decoder walks every detector-firing transform, ranks candidates by priority + printability. Ciphers deliberately have no detector (they look like plain letters), so pass them by name to `stegg_decode_transform` when suspected. See `AGENTS.md#text-transforms` and `docs/standard.md#transforms-vs-steg` for the transforms-vs-steg framing.
