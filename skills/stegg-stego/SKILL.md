---
name: stegg-stego
description: "ST3GG steganography via HTTP MCP server. Covers image (LSB, DCT/F5/JSteg, PVD, GIF, APNG, PDF, polyglots, audio-LSB, PNG chunk/EXIF injection, container carving), text and emoji (zero-width, cyrillic_homoglyph, cjk_homoglyph, whitespace, variation, combining, confusable, directional, hangul, mathbold, braille, emoji, skintone, capitalization, invisible_ink), network (PCAP steg + steganalysis), authorized jailbreak / prompt-injection composers and detectors, text transforms (Caesar, ROT13, Vigenère, Atbash, Base64, Base32, hex, binary, Morse, URL, quoted-printable, ASCII85, fullwidth, zalgo, homoglyph, invisible-text, zero-width, reverse, leetspeak) with a universal auto-decoder, and a typed knowledge base (records + prose corpus) for cited technique / survival / bibliography / pipeline lookups. Use when analyzing files for hidden content, hiding data in any of those carriers, running triage with SUSPICIOUS/INCONCLUSIVE/CLEAN verdicts, decoding a mystery encoded/ciphered string, probing what a stegg build supports at runtime, looking up cited numbers instead of guessing, or answering technique-tradeoff and transport-survival questions. Triggers on stegg, steganography, steg, LSB, DCT, F5, JSteg, PVD, hide data, hidden data, steganalysis, triage, zero-width, homoglyph, cyrillic_homoglyph, cjk_homoglyph, PNG chunk, EXIF injection, polyglot, PCAP steg, network steg, prompt injection, jailbreak, Unicode Tag, hidden emoji, capability probe, lookup technique, verify survival, verify claim, bibliography, pipeline design, knowledge base, caesar, rot13, vigenere, atbash, base64, base32, hex, morse, quoted-printable, ascii85, fullwidth, zalgo, leetspeak, auto-decode, universal decoder, text transform, encode text, decode text."
---
...
Streamable HTTP at `http://<host>:8765/mcp` after `pip install -e . && m3gast3gg-mcp` (MCP is core; no extra needed). One entry point covers three transports: `m3gast3gg-mcp --transport {stdio,sse,streamable-http}` (default: streamable-http; SSE served at `/sse` on the same port). See the top-level `README.md` (Quick Start + entry points) for stdio config, port flags, and auth caveats.
...
**Text transforms (`stegg_list_transforms` / `stegg_inspect_transform` / `stegg_encode_transform` / `stegg_decode_transform` / `stegg_chain_transforms` / `stegg_auto_decode`)** — reversible reshaping of a *bare* string, distinct from text steg (no cover, no secret). 26 transforms across 7 categories: ciphers (caesar, rot13, atbash, vigenere, bacon — no encryption; CTF-grade), encodings (base64, base32, base58, hex, binary, ternary, ascii85, morse, url, quoted-printable), unicode (fullwidth, zalgo), concealment bridges (homoglyph, invisible-text, zero-width — wrap the steg primitives into a single-arg transform), case (uppercase, lowercase, titlecase), format (reverse, remove-whitespace), visual (leetspeak). Reach for `stegg_auto_decode` when handed a mystery string and told "figure out what encoding this is" — the universal decoder walks every detector-firing transform, ranks candidates by priority + printability. Reach for `stegg_chain_transforms` when you need an ordered pipeline (e.g. caesar → base64 → zero-width for jailbreak-composition). Ciphers deliberately have no detector (they look like plain letters), so pass them by name to `stegg_decode_transform` when suspected. See `AGENTS.md#text-transforms` and `docs/standard.md#transforms-vs-steg` for the transforms-vs-steg framing.
